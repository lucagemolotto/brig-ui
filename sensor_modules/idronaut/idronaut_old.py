...
class coordinates:
	def __init__(self,label,lat,latdir,lon,londir,date_time):
		self.label=label
		self.lat=lat
		self.latdir=latdir
		self.lon=lon
...
last_position = coordinates(DEVICE_NAME,'unknown','unknown','unknown','unknown','unknown')
		...
		if string=='SO\n':
			...
			if s.write_line(string):
				...
				while line!='' and not shutdown:
					...
					if dataHarvesting:
						serial_data_lock.acquire()
						logFile.write(last_position.date_time+ ','+ last_position.lat+','+ last_position.latdir+','+ last_position.lon+','+ last_position.londir+'#'+line+'\n')
						serial_data_lock.release()
						logFile.flush()
					line=s.read_line()
			else:
